    ...
    describe('package.json', function () {
        ...
        describe('package.json JSON data', function () {
            it('must have valid name, description and author', function () {
                expect(json).to.have.property('name', 'ical-json');
                expect(json).to.have.property('description');
                expect(json.description).to.eql('JSON representation of calendar entries (with cron compatibility)');
                expect(json).to.have.property('author', 'Postman Labs <help@getpostman.com> (=)');
                expect(json).to.have.property('license', 'Apache-2.0');
                expect(json).to.have.property('homepage', 'https://github.com/postmanlabs/ical-json');
                expect(json).to.have.property('bugs', 'https://github.com/postmanlabs/ical-json/issues');

                expect(json).to.have.property('repository');
                expect(json.repository).to.eql({
                    type: 'git',
                    url: 'git://github.com/postmanlabs/ical-json.git'
                });

                expect(json).to.have.property('keywords');
                expect(json.keywords).to.eql(['ical-json', 'postman', 'ical', 'json', 'calendar', 'cron']);

                expect(json).to.have.property('engines');
                expect(json.engines).to.eql({ node: '>=4' });

                expect(json).to.have.property('preferGlobal', false);
            });
        ...
        describe('binary definitions', function () {
            it('must exits', function () {
                ...
                expect(json.bin).to.eql({ 'ical-json': './bin/index.js' });
            });
        });

        describe('script definitions', function () {
            it('files must exist', function () {
                var scriptPathRegex = /^node\snpm\/.*\.js$/;
                ...
                json.scripts && Object.keys(json.scripts).forEach(function (scriptName) {
                    ...
                    expect(scriptPathRegex.test(json.scripts[scriptName])).to.be.ok();
                });
            });
        });
